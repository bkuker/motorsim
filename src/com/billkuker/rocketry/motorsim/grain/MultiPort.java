...
public class MultiPort extends ExtrudedShapeGrain implements Validating {
	...
	private void generateGeometry(){
		...
		for ( int i = 0; i < r1Count; i++ ){
			Shape port = new Ellipse2D.Double(r1off-r1dmm/2,-r1dmm/2,r1dmm,r1dmm);
			xsection.subtract(port, AffineTransform.getRotateInstance(i*(2.0*Math.PI/r1Count)));
		}
		
		for ( int i = 0; i < r2Count; i++ ){
			Shape port = new Ellipse2D.Double(r2off-r2dmm/2,-r2dmm/2,r2dmm,r2dmm);
			xsection.subtract(port, AffineTransform.getRotateInstance(r2rot + i*(2.0*Math.PI/r2Count)));
		}
	}
